...
#'
#' @description This function calculates metrics (D, Dx, V) based on a fitted Cox model using the `cph` function from
#' the `rms` package. It uses survival estimates to assess the concordance between observed and predicted
#' survival outcomes.
#'
#' @param train.fit A fitted Cox model from `rms::cph`.
#' @param traindata A data frame containing training data.
#' @param newdata A data frame with new data for prediction.
#' @return A list containing the model call, D, Dx, V metrics, and other calculated components.
#' @references
...
pam.schemper <- function (train.fit, traindata, newdata) {
    ...
    km.fit <- survival::survfit(survival::Surv(time, status) ~ 
                                  1, data = traindata)
    tempi.eventi <- km$time[km$n.event != 0]
    pos.surv <- apply(as.matrix(tsurv), 1, f.assegna.surv, tempi.eventi)
    surv.tj <- stats::approx(km.fit$time, km.fit$surv, xout = tempi.eventi, 
                             method = "constant", f = 0, yleft = 1, yright = min(km.fit$surv, 
                                                                                 na.rm = T))$y
    ...
    surv0.tj.cox <- stats::approx(train.fit$time, train.fit$surv, 
                                  xout = tempi.eventi, method = "constant", f = 0, yleft = 1, 
                                  yright = min(km.fit$surv, na.rm = T))$y
    ...
    Mtx <- apply(as.matrix(tempi.eventi), 1, f.Mt.cox, tsurv, 
                 ...
                 num.sogg, lin.pred)
    Gkm <- survival::survfit(survival::Surv(tsurv, ind.censura) ~ 
                               1)
